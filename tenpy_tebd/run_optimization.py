...
if __name__ == "__main__":
    ...
    try:
        ...
        ### Initialization from chi/2 training.
        init_path = 'data/1d_TFI_g%.4f_h%.4f/L31_chi32/T%.1f/circuit_depth%d_Niter100000_1st_circuit.pkl' % (g, h, T, depth)
        my_circuit = pickle.load(open(init_path, 'rb'))
        ### Initialization from depth-1, and adding an extra layer.
        # init_path = 'data/1d_TFI_g%.4f_h%.4f/L31_chi64/T%.1f/circuit_depth%d_Niter100000_1st_circuit.pkl' % (g, h, T, depth-1)
        # my_circuit = pickle.load(open(init_path, 'rb'))
