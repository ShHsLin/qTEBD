...
if __name__ == "__main__":
    ...
    if H == 'TFI':
        ...
        # H = xx + gz
        H = ed.get_H_Ising(g, h, J, L, change_basis=True)
    elif H == 'XXZ':
    ...
    for i in range(L//2):
        init_state = np.kron(init_state, down_state)
    ...
    for site in range(L):
        sz_array[0, site] = ed.Op_expectation(Sz, site, psi, L)

    for i in range(1, int(total_time // dt) +  1):
        psi = scipy.sparse.linalg.expm_multiply(-1.j*dt*H, psi)
        for site in range(L):
            sz_array[i, site] = ed.Op_expectation(Sz, site, psi, L)
    ...
    plt.figure()
    plt.imshow(1- sz_array, aspect='auto')
    plt.figure()
    plt.plot(sz_array[:,L//2])
    plt.show()
